...
**验证标准**：启动应用 → 看到 5 个预设工具 → 修改路径 → 点击扫描 → 看到 Skill 列表。
...
| 功能 | PRD 章节 |
|------|---------|
| SSOT 目录创建（`~/.agents/skills/local/`） | §5.5 |
| 单向同步（SSOT → 工具目录） | §5.5 |
| 双向同步（工具 → SSOT → 其他工具） | §5.5 |
| `local.md` 标记机制 | §5.5 |
| 启用/禁用 Skill | §5.6 |
| 半自动 / 全自动模式切换 | §5.5 |
| "检查更新"按钮 | §7 |
...
**目标**：项目管理、操作反馈、错误处理全部就绪，MVP 可交付。

| 功能 | PRD 章节 |
|------|---------|
| 项目管理（添加/删除/切换） | §5.7 |
| 项目级 Skill 隔离 | §5.7 |
| 操作反馈（Toast/Dialog） | §5.8, §16 |
| 错误处理（所有边界情况） | §15 |
| 加载态 / 空态 UI | §16.2, §16.3 |
| 同步日志查看 | §5.4 |
| 路径格式容错全量测试 | §14 |
